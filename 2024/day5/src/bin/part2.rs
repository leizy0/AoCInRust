...
fn main() -> Result<()> {
    ...
    let page_n_sum = updates
        ...
        .map(|up| {
            let corrected_up = rules.correct(up);
            *corrected_up.get(corrected_up.len() / 2).unwrap_or(&0)
        })
        .sum::<usize>();
    println!(
        "The sum of middle page number in updates which are corrected by given rules is {}.",
        page_n_sum
    );
